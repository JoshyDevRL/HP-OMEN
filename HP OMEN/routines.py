...
class shadow_defend():
    #stays behind the ball until the optimal time to challenge
    def run(self,agent):
         x = 1
         agent.controller.boost = False
         if x == 1:
            target = Vec3(agent.ball.location.x, agent.ball.location.y + 1500*side(agent.team), agent.ball.location.z)
            local_target = agent.me.local(target - agent.me.location)
            defaultPD(agent, local_target)
            defaultThrottle(agent, 2300)
            agent.line(local_target, agent.me.location)
         if agent.me.location == local_target:
            target = Vec3(agent.ball.location.x, agent.ball.location.y + 1000*side(agent.team), agent.ball.location.z)
            local_target = agent.me.local(target - agent.me.location)
            defaultPD(agent, local_target)
            defaultThrottle(agent, 2300)
            agent.line(local_target, agent.me.location)
         elif agent.me.location == local_target:
            target = Vec3(agent.ball.location.x, agent.ball.location.y + 500*side(agent.team), agent.ball.location.z)
            local_target = agent.me.local(target - agent.me.location)
            defaultPD(agent, local_target)
            defaultThrottle(agent, 2300)
            agent.line(local_target, agent.me.location)
