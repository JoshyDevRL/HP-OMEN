...
class omen(GoslingAgent):
    def run(agent):
        ...
        def shadow_defend(agent):
            agent.push(goto(agent.friend_goal.location))
            if abs(agent.me.location.y) > abs(agent.ball.location.y) + 450:
                agent.push(short_shot(agent.foe_goal.location))
            elif abs(agent.foe_goal.location.y) < abs(agent.me.location.y) + 200:
                agent.push(short_shot(agent.foe_goal.location))
            return
        ...
        if len(agent.stack) < 1:
            ...
            if agent.ball.location.y < 0 and agent.team == 0:
                shadow_defend(agent)

            elif agent.ball.location.y > 0 and agent.team == 1:
                shadow_defend(agent)

                    
            if (close and me_onside) or (not foe_onside and me_onside):
